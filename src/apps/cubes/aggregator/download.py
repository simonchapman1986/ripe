...
from apps.cubes.models import CubeDownload

from apps.cubes import aggregator as aggregator


class DownloadCube(aggregator.AggregatorFactory):
    """
...
def build_aggregate(date=None):
    b = DownloadCube(fact=FactServicesStorefrontDownload, agg_fact=CubeDownload, date=date, ignore_platform=True)
    b.set_values(
